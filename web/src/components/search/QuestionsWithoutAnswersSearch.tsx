...
} from "@/components/search/FormPeriodSearchForm"
import { searchService } from "@/services/search-service"
import type { Form } from "@/types/form"
import type { QuestionWithoutAnswer } from "@/types/search"

export function QuestionsWithoutAnswersSearch({ forms }: { forms: Form[] }) {
  ...
  async function handleSearch({ formId, start, end }: PeriodSearchValue) {
    ...
    try {
      setQuestions(
        await searchService.getQuestionsWithoutAnswers(formId, start, end)
      )
    } catch (requestError) {
  ...
  return (
    <div className="grid gap-4">
      <p className="text-sm text-muted-foreground">
        Encontre perguntas que ainda não receberam respostas dentro do período
        selecionado.
      </p>
      <FormPeriodSearchForm
        forms={forms}
      ...
      />
      {isSearching ? <LoadingState label="Verificando perguntas pendentes…" /> : null}
