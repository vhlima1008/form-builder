...
import type { OwnerFormResponse } from "@/types/search"

export function ResponsesInPeriodSearch({ forms }: { forms: Form[] }) {
  ...
  async function handleSearch({ formId, start, end }: PeriodSearchValue) {
    ...
    setError(null)
    try {
      setResponses(await searchService.getResponsesInPeriod(formId, start, end))
    } catch (requestError) {
  ...
  return (
    <div className="grid gap-4">
      <p className="text-sm text-muted-foreground">
        Localize respostas recebidas em um intervalo específico para acompanhar
        atividade recente ou períodos de campanha.
      </p>
      <FormPeriodSearchForm
        forms={forms}
